...
public class TruckSeat : Interactable
{
    ...
    private void OnOccupantChanged(GameObject oldOccupant, GameObject newOccupant)
    {
        ...
        if (oldOccupant != null)
        {
            ...
            oldOccupant.GetComponent<Rigidbody>().WakeUp(); // Wake up the old occupant's Rigidbody to resume physics simulation
        }
        if (newOccupant != null)
        {
            ...
            newOccupant.GetComponent<Rigidbody>().Sleep(); // Put the new occupant's Rigidbody to sleep to prevent physics issues
        }
    }
